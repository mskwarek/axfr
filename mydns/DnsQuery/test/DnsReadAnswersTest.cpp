...
TEST_F(DnsReadAnswersTest, testReadingZonetransferMe)
{
    ...
    unsigned char *reader = &dnsByteBuffer[sizeof(struct DNS_TCP_HEADER) +
                                           (strlen((const char*)&dnsByteBuffer[sizeof(struct DNS_TCP_HEADER)])+1) + sizeof(struct QUESTION)];
    EXPECT_FUNCTION_CALL(printToFilelMock, (_, _, _, _)).WillRepeatedly( Invoke( [](auto, auto, auto, auto){} ));
    FILE f = {0};
    readAnswers(TRANSPORT_TYPE_TCP, reader, answers, dnsByteBuffer, &f, get_answers_cnt(&dns->header));
}
