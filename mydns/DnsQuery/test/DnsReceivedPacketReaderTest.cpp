...
{
    {
        ...
        FILE f;
        EXPECT_FUNCTION_CALL(fileMock, (_, _)).WillOnce(Return(&f));
        EXPECT_FUNCTION_CALL(printToFileMock, (_, _, _, _)).WillRepeatedly(Invoke([](auto, auto, auto, auto){}));
        EXPECT_FUNCTION_CALL(readAnswersMock, (_, _, _, _, _, _)).WillRepeatedly(
                Invoke([](auto, auto, auto, auto, auto, auto){}));
